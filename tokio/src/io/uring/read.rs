...
impl Op<Read> {
    ...
    pub(crate) fn read(file: Arc<StdFile>, buf: Buf, file_offset: u64) -> io::Result<Self> {
        ...
        let ptr = buf.bytes().as_ptr() as *mut u8;

        let sqe = opcode::Read::new(types::Fd(file.as_raw_fd()), ptr, len)
            .offset(file_offset)
            .build();
        ...
        let op = unsafe { Op::new(sqe, Read { buf, file }) };
        Ok(op)
    }
}
